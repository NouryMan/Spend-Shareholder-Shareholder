...
namespace Spend_Web_System.API
{
    ...
    public class API_SALES_INVController : ApiController
    {
                ...
                using (TransactionScope trns = new TransactionScope())
                {
                    ...
                    if (result > 0)
                    {
                        ...
                        if (ACCList.Count() <= 0)
                        {
                            ...
                            PERSON_ACC.NOTE = "عميل شقة بواسطة النظام";
        ...
        public IHttpActionResult Add([FromBody] SALES_INVTBL_Model model)
        {
            ...
            model.QRCODE = QrCode.encodeQrText("فالح الحازمي", "310195291300003", model.DATE_M.Value.ToString("u"), model.INV_PUR.ToString(), model.VAT_AMOUNT.ToString());
        ...
        public IHttpActionResult Edite([FromBody] SALES_INVTBL_Model model)
        {
            ...
            model.QRCODE = QrCode.encodeQrText("فالح الحازمي", "310195291300003", model.DATE_M.Value.ToString("u"), model.INV_PUR.ToString(), model.VAT_AMOUNT.ToString());
            model.PROJ_NO = model.SALES_INVDTTBL_Collection.FirstOrDefault().PROJ_NO;
            model.PART_PROJ = model.SALES_INVDTTBL_Collection.FirstOrDefault().PROJ_NO;
            model.UNIT_NO = model.SALES_INVDTTBL_Collection.FirstOrDefault().PROJ_NO;
