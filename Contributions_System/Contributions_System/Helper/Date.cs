...
namespace Contributions_System.Helper
{
    public class Date
    {
           ...
           public string ConvertToFullDate(string d)
             {
            if (d.Length <= 0)
            {
                cur.Trace.Warn("GregToHijri :Date String is Empty");
            ...
            DateTime tempDate = DateTime.ParseExact(d, allFormats,
                        enCul.DateTimeFormat, DateTimeStyles.AllowWhiteSpaces);
            var d1 = " يوم " + ConvertDayTArabic(tempDate.ToString("dddd")) + " بتاريخ " + GregToHijri(d) + "هـ الموافق " + d + "م ";
            return d1;
             }
           

        public string ConvertDayTArabic(string day)
        {
            if (day.Length <= 0)
            {
                cur.Trace.Warn("GregToHijri :Date String is Empty");
